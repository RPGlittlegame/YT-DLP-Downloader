...
class App(ctk.CTk):
    ...
    def _set_app_icon(self):
        ...
        png_path = os.path.join(base_dir, 'YDD.icon', 'Assets', 'square.and.arrow.down.fill.png')
    ...
    def create_widgets(self):
        ...
        self.v_quality_combo = ctk.CTkComboBox(self.options_frame, variable=self.v_quality_var,
                                               values=["自动 (Best)"], corner_radius=6, width=160)
        ...
        self.format_combo = ctk.CTkComboBox(self.options_frame, variable=self.format_var,
                                            values=["MP4", "MKV", "AVI", "WMV", "MOV", "MP3"], corner_radius=6, width=160)
        ...
        self.output_btn = ctk.CTkButton(self.options_frame, text="选择目录", fg_color="transparent",
                                        border_width=1, text_color=("gray10", "#DCE4EE"), corner_radius=6,
                                        command=self.choose_output_dir, width=160)
        ...
        self.sub_combo = ctk.CTkComboBox(self.options_frame, variable=self.sub_var,
                                         values=["无字幕"], corner_radius=6, width=160,
        ...
        self.cookie_frame.grid(row=3, column=1, columnspan=3, padx=5, pady=(0, 10), sticky="ew")
        ...
        self.cookie_import_btn = ctk.CTkButton(self.cookie_frame, text="导入 Cookie", width=90,
                                               corner_radius=6, command=self.import_cookie_file)
        ...
        self.size_label.grid(row=0, column=0, columnspan=2, sticky="w", padx=2, pady=(0, 4))

        self.download_btn = ctk.CTkButton(self.action_frame, text="开 始 下 载",
                                          font=ctk.CTkFont(size=14, weight="bold"),
                                          height=35, corner_radius=6, command=self.on_download)
        self.download_btn.grid(row=1, column=0, sticky="ew", padx=(0, 5))

        self.cancel_btn = ctk.CTkButton(self.action_frame, text="取消",
                                        font=ctk.CTkFont(size=14, weight="bold"),
    ...
    def update_size_estimate(self, *_):
        ...
        if has_any_size:
            ...
            self.size_label.configure(text=f"📦 预估大小: ~{size_str}", text_color=("gray30", "gray70"))
        else:
            self.size_label.configure(text="📦 预估大小: 未知（平台未提供）", text_color="gray")
        ...
        def fetch_task():
            ...
            def update_ui():
                if res['status'] == 'success':
                    ...
                    if sub_count > 0:
                        self.update_log(f"✅ 检测到 {sub_count} 个字幕轨道，可在字幕选项中选择。")
                    else:
                    ...
                    self.size_label.configure(text="📦 预估大小: 解析后显示", text_color="gray")
